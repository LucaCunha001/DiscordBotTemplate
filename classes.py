...
SORTEIOS_PATH = "sorteios.json"

def carregar_dados() -> dict:
	if not os.path.exists(SORTEIOS_PATH):
		return {}
	with open(SORTEIOS_PATH, "r", encoding="utf-8") as f:
		return json.load(f)

def salvar_dados(dados):
	with open(SORTEIOS_PATH, "w", encoding="utf-8") as f:
...
class CallCreatorView(discord.ui.View):
	...
	def __init__(self, bot: discord.Client):
		...
		super().__init__(timeout=None)
		self.add_item(CallCreator(bot=bot))
...
class DropdownView(discord.ui.View):
	...
	"""
	def __init__(self, bot: discord.Client, dropdown=Dropdown):
		super().__init__(timeout=None)
...
class SorteioView(discord.ui.View):
	...
	async def participar(self, interaction: discord.Interaction, button: discord.ui.Button):
		...
		sorteio = dados.get(str(interaction.message.id), {})
		participantes = set(sorteio.get("participantes", []))
		...
		if interaction.user.id in participantes:
			...
			sorteio["participantes"] = list(participantes)
			dados[str(interaction.message.id)] = sorteio
			salvar_dados(dados)
			await interaction.response.send_message("Você entrou no sorteio!", ephemeral=True)
		
		embed = interaction.message.embeds[0]
		embed.set_footer(text=f"{int(embed.footer.text.split(" ")[0])+1} participantes" if embed.footer else "0 participantes")
