...
lista: list[app_commands.Choice[str]] = []
...
class ComandosStaff(app_commands.Group):
    ...
    def has_manage_roles_or_specific_role(self):
    ...
    @app_commands.command(name="imagens_setup", description="Mostra quais imagens são enviadas em cada promoção", guild=discord.Object(id=1288221625732431957))
        ...
        try:
            ...
            if isinstance(motivo, str):
                if motivo.lower().startswith("saiu"):
                    ...
                    embed.set_image(url=self.pegar_imagem(role, False))
            ...
            embed.timestamp = datetime.datetime.now(datetime.timezone.utc)
            url = await self.pegar_imagem(role, str(promoveu))
    ...
    class PromoverAgora(discord.ui.View):
        def __init__(self, interaction: discord.Interaction, membro: discord.Member, cargo: int, motivo: str):
        ...
        async def promover_callback(self, interaction: discord.Interaction, button: discord.ui.Button):
            await ComandosStaff.promover_act(interaction=interaction, membro=self.membro, cargo=self.cargo, motivo=self.motivo)
    ...
    async def promover(self, interaction: discord.Interaction, membro: discord.Member, cargo: str, motivo: str=None):
        ...
        if tempo_falta:
            ...
            if tempo_desde_ultima < tempo_minimo:
                ...
                await interaction.followup.send(f"Olha, {interaction.user.mention}, o {membro.mention} foi promovido <t:{int(ultima_promocao.timestamp())}:R>. Recomendo promover <t:{int(timestamp_falta)}:R>. Deseja fazer isso agora?", view=self.PromoverAgora(interaction, membro, cargo, motivo))
                return
    ...
    class ConviteView(discord.ui.View):
        def __init__(self, precisa_form: bool, membro: discord.Member, interaction: discord.Interaction):
            ...
            if precisa_form:
                ...
                self.add_item(self.PromoverButton(membro, interaction))
    
    class PromoverButton(discord.ui.Button):
        def __init__(self, membro: discord.Member, interaction: discord.Interaction):
            super().__init__(label="Aceitar na Staff", style=discord.ButtonStyle.success)
            self.membro = membro
            self.interaction = interaction
    
        async def callback(self, interaction: discord.Interaction):
            await self.promover_act(self.interaction, self.membro, constantes.CARGOS.CARGOS_DE_PROMOCAO[1], "Convidado")
    ...
    async def convidar(self, interaction: discord.Interaction, membro: discord.Member, precisa_form: bool, texto: str = None):
        ...
        view = self.ConviteView(precisa_form, membro, interaction)
        embed = discord.Embed(
    ...
    @app_commands.checks.has_permissions(administrator=True)
    async def ticket_treina(interaction: discord.Interaction):
        ...
        if not canal_existente:
            ...
            view=TicketView(), content=f"{interaction.user.mention}")
            embed = discord.Embed(
                title="Ticket criado!",
    ...
    async def tipo_autocomplete(interaction: discord.Interaction, current: str) -> list[app_commands.Choice]:
